...
/* based on https://github.com/mcmtroffaes/inipp */
#pragma once
...
public:
			...
			if (length > 0) {
				...
				if (front == char_comment) {
					...
					if (line.back() == char_section_end)
						...
						std::cout<<"section line "<<line<<" incorrect."<<std::endl;
					...
					if (sec.find(variable) == sec.end()) {
						...
						errors.push_back(line);
						std::cout<<"multiply defined "<<variable<<" found!"<<std::endl;
						std::cout<<"exit!"<<std::endl;
						exit(0);
					}
				}
    ...
    /* insert cfg of the default section into other sections */
